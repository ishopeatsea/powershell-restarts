...
while (1) {
    ...
    if ($lab.SubString(0,2) -match "CB") {
        ...
        Write-Host "Invalid format." # what about e.g. 05D1021, 0804C01
        continue
    }
    ...
    foreach ($computer in $computers) {
        ...
        if (Test-NetConnection -ComputerName $computer -InformationLevel Quiet) {
            ...
            #         $successful++
            #     }
            $result = Invoke-Command -ComputerName $computer {gpupdate}
            $fails = $result | Select-String -Pattern 'fail' -SimpleMatch | Select-String -Pattern 'AppV' -NotMatch -SimpleMatch
            $successes = $result | Select-String -Pattern 'success' -SimpleMatch
            Write-Host "$computer :: Completed."
            if ($successes) { Write-Host $successes -ForegroundColor Green }
            if ($fails) { Write-Host $fails -ForegroundColor Red }
